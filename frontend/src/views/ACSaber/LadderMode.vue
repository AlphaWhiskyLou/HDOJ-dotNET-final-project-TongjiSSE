  ...
  <div>
    <div class="ac-saber-menu-ladder">
      ...
      <div class="ac-saber-menu-ladder-footer">
        <div class="ac-saber-menu-ladder-footer-start" @click="matchUser()" v-show="!isMatching&&isMatchingButtonShow">
          开始匹配
        </div>
        <div class="ac-saber-menu-ladder-footer-start" @click="cancelMatch()" v-show="isMatching&&isMatchingButtonShow">
          取消匹配
        </div>
      </div>
  ...
  methods: {
    ...
    initWebsocket() {
      ...
      thus.ws.onmessage = function (msg) {
        ...
        if (message.chatMessage.type == 'MATCH_USER') {
...
// 游戏中
    ...
    updateSoloScoreInfo(){
      let formData = new FormData();
      formData.append('winnerId',this.userId)
      formData.append('loserId',this.opponentInfo.userId)
      axios.post('/solo/updateWinnerAndLoser',formData).then((res)=>{
        console.log(res)
      }).catch((err)=>{
        console.log(err)
      })
