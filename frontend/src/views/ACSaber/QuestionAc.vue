  ...
  <el-container>
    <el-container>
            ...
            <el-card style="">
              <el-card-content v-if="this.result!=''">
                ...
                <div v-if="this.result.code == 200">
                  ...
                  通过测试用例： <span>{{ this.result.data.allCases }}</span> /
                  <span>{{ this.result.data.allCases }}</span><br>
                </div>
                <!-- 运行失败 -->
                <div v-else>
                  执行结果： <span>未通过</span><br>
                  执行用时： <span>{{ this.result.data.time }}</span><br>
                  通过测试用例： <span>{{ this.result.data.testCases }}</span> /
                  <span>{{ this.result.data.allCases }}</span><br>
      ...
      <el-main style="overflow:hidden">
        <div>
          <MonacoEditor ref="monacoEditor" :myArgs="myArgs" style="width: 800px;height: 1000px"/>
        </div>
      </el-main>
    </el-container>
    <el-divider></el-divider>
  ...
  methods: {
    ...
    submit() {
      ...
      this.$http.post("/question/check", {
        "code": newcode === undefined || newcode===""? 'empty' : newcode,
        'questionId': this.id,
